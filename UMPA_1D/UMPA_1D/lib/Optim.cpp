                ...
                *args = args_copy;
                ...
                switch(obj->subpx_func){ // sara: add possibility of choosing sub_pixel optimization method
                    ...
                    case 1: *out = spmin_quad(a, uv); // sara: quadratic fit, supposedly same as in pythonUMPA
                            break;
                    default:
                            *out = spmin_quad(a, uv); // fdm: the call modifies uv in-place.
                    }
...
template <typename T, typename OBJ, typename COST_ARGS, error_status(OBJ::* f)(T*, int*, COST_ARGS* args)>
error_status discrete_1d_minimizer(T* out, T* uv, OBJ* obj, COST_ARGS* args, minimizer_debug<T>* db) {

    T cost_current, cost_m, cost_p;
    T a, b, c;               // coefficients of the fitted parabola
    int ij[2], ij_m[2], ij_p[2];
    int Ncalls = 0;          // number of cost function evaluations
    int min_m, min_p;        // bool to describe if new evaluated point is smaller than current value.
    int min_dir = 0;         // 0 if unknown, -1 or +1 give direction of found local minimum
    T tol = 1e-8;            // A small number to get out of cases where neighboring pixels have the same value
    error_status s;
    COST_ARGS args_copy;
    ...
    s = (obj->*f)(&cost_current, ij, args);
    if (!s.ok) return s;
    args_copy = *args;  // args contains transmission and dark-field estimates and thus needs to be copied
    Ncalls += 1;

    ij_m[0] = ij[0];
    ij_p[0] = ij[0];
    printf("run 1d minimizer\n");
    while (Ncalls < MAX_CALLS) {
        
        ij_m[1] = ij[1] - 1;  // Looking west: (0, -1)

        s = (obj->*f)(&cost_m, ij_m, args);       // fdm: "s, d[c_m] = f(ij_m, args)"
        if (!s.ok) return s;
        Ncalls += 1;
        
        min_m = cost_m > cost_current + tol;
        if (!min_m) args_copy = *args;  // If the value lower, copy output values

        ij_p[1] = ij[1] + 1;  // Looking east: (0, 1)

        s = (obj->*f)(&cost_p, ij_p, args);
        if (!s.ok) return s;
        Ncalls += 1;

        min_p = cost_p > cost_current - tol;
        if (!min_p) args_copy = *args;  // If the value lower, copy output values

        if (min_m & min_p) // fdm: "&" is bitwise AND
        {
            // Found minimum
            min_dir = cost_m < cost_p ? -1 : 1;
            *args = args_copy;

            // calculate minimum of parabola here, send cost function value to out
            a =  0.5 * cost_m - cost_current + 0.5 * cost_p;
            b = -0.5 * cost_m                + 0.5 * cost_p;
            c =                 cost_current;

            uv[1] = -b / (2 * a);
            *out = uv[1] * 0.5 * b + c;

            uv[1] += ij[1];  // convert the relative position of the minimum to an absolute one.

            return s;
        }

 
        uv[1] = ij[1];  // not sure about this... todo
        ...
        *out = cost_current;
        ...
        if (min_m)  // implied: min_p = false! (because we already covered the
                    // possibilites min_p = min_m = False and min_p = min_m = True
        {
            // Move in positive direction
            ij[1] += 1;
        ...
        else {  // i.e.: min_p = True, min_m = False
            ij[1] -= 1;
